...
export default function PricingPage() {
  ...
  const onSubscribe = async (priceId: string | null, planId: string) => {
    ...
    if (!priceId) { toast.error('Payment not yet configured.'); return; }
    setLoading(planId);
    ...
    <div className="space-y-6 pb-8 max-w-3xl">
            ...
            >
              ...
              <div className="mt-8">
                {isCurrent ? (
                  ...
                  >
                    {loading === plan.id ? 'Loading…' : `Upgrade to Pro — €${plan.price}/month`}
      ...
      <Card className="p-5">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10"><Zap className="h-4.5 w-4.5 text-primary" /></div>
          <div>
            <p className="text-sm font-medium">Questions?</p>
            <p className="text-xs text-muted-foreground">Email us at <a href="mailto:support@snapsell.ai" className="text-primary hover:underline">support@snapsell.ai</a></p>
          </div>
        </div>
      </Card>
    </div>
